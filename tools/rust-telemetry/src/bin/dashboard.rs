...
.card { padding: 1.5rem; border-radius: 1rem; background: #111a33; box-shadow: 0 10px 30px rgba(0,0,0,0.4); max-width: 720px; }
h1 { margin-top: 0; }
...
.metric span { display: block; font-size: 0.85rem; color: #9fb3c8; text-transform: uppercase; letter-spacing: 0.08em; }
.metric strong { font-size: 1.6rem; color: #72e0ff; }
...
async function refresh() {
  ...
  document.getElementById('history').textContent = historyJson
    .slice(-20)
    .map((item) => `${item.timestamp} raw=${item.raw} voltage=${item.voltage.toFixed(3)} V`)
    .join('\n');
}
